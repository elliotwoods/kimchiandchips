...
PCDecode::PCDecode(GUI *screens, PayloadBase *payload, Camera *camera, ofTexture &output)
{		
	_screens = screens;
	_payload = payload;
	_camera = camera;
	...
	_scrSend = new scrTexture(_screens->nScreens(), cursor_xy, false, 0, output, _texCameraSpacePreview, "Output / Cameraspace");
	_screens->addScreen(*_scrSend);
	ofAddListener(_scrSend->evtCursorMove, this, &PCDecode::moveSendCursor);
	
	_scrFrameData = new scrTexture(_screens->nScreens(), cursor_none, true, 1, *_texFrameDataPreview, _texProjectorSpacePreview, "Frame data / Projectorspace");
	_screens->addScreen(*_scrFrameData);
	
	_scrBinary = new scrTexture(_screens->nScreens(), cursor_none, false, 2, *_texBinary, "Binary image");
	_screens->addScreen(*_scrBinary);
	
	
	_scrThreshold = new scrTexture(_screens->nScreens(), cursor_none, false, 3,*_texThresholdMasked, "Threshold (masked)");
	_screens->addScreen(*_scrThreshold);
	
	_scrHistograms = new scrHistograms(_screens->nScreens(), cursor_none, false, 4, "Histograms");
	_screens->addScreen(*_scrHistograms);
	_scrHistograms->addHistogram(*_histThresholdRange);
	_scrHistograms->addHistogram(*_histNFinds);
	
	_scrCamera = new scrTexture(_screens->nScreens(), cursor_none, false, 5, *_texCamera, "Camera");
	_screens->addScreen(*_scrCamera);
...
void PCDecode::capture()
{
	bool isCamRGB;
	
	_charCamera = _camera->capture();
	...
	for (iCamPixelX = 0; iCamPixelX < camWidth; iCamPixelX++)
		for (iCamPixelY=0; iCamPixelY < camHeight; iCamPixelY++) {
			...
			if (_boolThresholdMask[iCamPixel]) // DOES THIS PIXEL FIT WITHIN SELECTED THRESHOLD MIN RANGE
			{
				...
				_payload->decode(_intFrameData[iCamPixel],
					iProjectorInterleavePixelX, iProjectorInterleavePixelY,
					isValid);
