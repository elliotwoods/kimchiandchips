...
bool PCConfig::configLoad(string filename)
{
	...
	if (success)
	{
		try
		{
			...
			{
				camWidth = _configFileXML.getAttribute("camera", "width", 640, 0);
				camHeight = _configFileXML.getAttribute("camera", "height", 480, 0);
				captureDelay = _configFileXML.getAttribute("camera", "delay", 100, 0);
				exposure = _configFileXML.getAttribute("camera", "exposure", CONFIG_DEFAULT_EXPOSURE, 0);
				gain = _configFileXML.getAttribute("camera", "gain", CONFIG_DEFAULT_GAIN, 0);
